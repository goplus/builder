...
到目前为止，我们写的代码都是直接放在最上面，游戏一开始就自动执行：
...
**事件**是指"发生的事情"，当它发生时，我们执行相应的代码来**响应**它：
- 游戏开始运行（onStart） → 开始事件
- 玩家按下键盘（onKey） → 键盘事件
- 玩家点击鼠标 (onClick) → 鼠标事件
...
在前面的课程里，我们把命令直接写在最上面，游戏一运行就执行。其实，"游戏开始"本身就是一个**事件**，而 `onStart` 就是用来响应这个事件的事件处理器。这一节我们正式认识它。

#### onStart 是什么？

`onStart` 是一个**事件处理器**（Event Handler），表示"当游戏开始运行时"要执行的代码。你可以把它想象成游戏的"发令枪"：
- 当你点击"运行"按钮时，游戏就开始了
- `onStart => { ... }` 大括号 `{}` 里的代码会在这时**自动执行**
...
两者运行效果相同。对于简单的程序，直接写更简洁；当程序变复杂时，`onStart` 能让"开始时要做的事"更清晰，并且和其它事件（`onKey`、`onClick` 等）的写法保持统一。

#### 为什么需要 onStart？

1. **多段启动逻辑**：可以写多个 `onStart`，把不同的初始化分开管理。
2. **与其它事件并存**：`onStart` 和 `onKey`、`onClick` 是平级的事件处理器；把"开始时要做的事"放进 `onStart`，整段代码结构更统一。
3. **非阻塞**：某个 `onStart` 里的等待不会卡住其它事件的响应。

#### 试一试

> 课程地址：https://x.qiniu.com/editor/curator/Coding-Course-1/sprites/Kiko/code

把第一个程序里的 `step 160` 用 `onStart => { ... }` 包起来，再点击运行，效果和之前完全一样：
...
| 概念 | 说明 | 示例 |
|------|------|------|
| `onStart` | 游戏开始时执行的事件处理器 | `onStart => { step 160 }` |
| 事件处理器 | 响应某个事件的代码块 | `onStart` / `onKey` / `onClick` |
...
**下一节预告**：我们已经学会了响应单个按键，但如果要实现完整的方向控制呢？玩家需要能够控制角色向上、下、左、右四个方向移动。接下来的两节将学习如何实现**完整的方向控制系统**！
...
**下一节预告**：我们已经实现了基本的方向控制，但游戏还需要更加多的操作。下一节将学习如何处理移动以外的动作，让游戏变得更加好玩！
