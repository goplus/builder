...
import {
  ApiException,
  ApiExceptionCode,
} from "../../../spx-gui/src/apis/common/exception";
...
class RecordingServiceImpl implements RecordingService {
  async createRecording(
    ...
    return client.post("/recordings", params, {
      signal,
    }) as Promise<RecordingData>;
  }

  async getRecording(id: string, signal?: AbortSignal): Promise<RecordingData> {
    return client.get(`/recording/${id}`, undefined, {
      signal,
    }) as Promise<RecordingData>;
  }
  ...
  async updateRecording(
    ...
    return client.put(`/recording/${id}`, params, {
      signal,
    }) as Promise<RecordingData>;
  }
  ...
  ): Promise<ByPage<RecordingData>> {
    return client.get("/recordings/list", params) as Promise<
      ByPage<RecordingData>
    >;
  }
}
...
export const createRecording =
  recordingService.createRecording.bind(recordingService);
export const getRecording =
  recordingService.getRecording.bind(recordingService);
export const updateRecording =
  recordingService.updateRecording.bind(recordingService);
export const deleteRecording =
  recordingService.deleteRecording.bind(recordingService);
export const listRecording =
  recordingService.listRecording.bind(recordingService);

// Recording Interaction Service functions
export const recordRecordingView =
  recordingInteractionService.recordRecordingView.bind(
    recordingInteractionService
  );
export const isLikingRecording =
  recordingInteractionService.isLikingRecording.bind(
    recordingInteractionService
  );
export const likeRecording = recordingInteractionService.likeRecording.bind(
  recordingInteractionService
);
export const unlikeRecording = recordingInteractionService.unlikeRecording.bind(
  recordingInteractionService
);
