...
API 视频按 definition id 索引，并按用户记录已学过的，所以**同一个概念不会被讲第二遍**；同一 API 的任意重载解析到同一个视频，非 API 的主题也可以有视频（尺子——用户问"怎么量"时 copilot 会播，它是从 `<api-video>` 的说明里知道这个 ID 的，别处都没有）。弹窗抽成共享的 `ApiVideoModal`。
