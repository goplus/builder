...
素材是对项目中独立发布的可复用内容的统称，内容的格式有多种，如 Sprite、Sound、Backdrop 等。用户在创作游戏的过程中，可以把这些相对零碎的可复用的内容独立发布，以允许自己在别的项目、或别人在别的项目复用之。
...
对于尚未保存到云端的变更，编辑器会将其暂存在用户浏览器中。即，如果用户做了变更 A，且未将其保存到云端，随后关闭了编辑器。再次使用编辑器打开该项目时，变更 A 不会被丢弃，用户可以继续在 A 的基础上继续编辑。这个暂存是可能失效的，失效的原因包括但不限于：
...
用户可以通过编辑器将 Project 中的 Game 导出为一个文件并保存到用户本地。

用户也可以在编辑器中选择导入文件，这个文件应当是自己或者别人之前通过编辑器的“导出”得到的。导入文件会将文件对应的 Game 信息恢复到当前编辑器中。

因为导入导出的内容只包含 Game 信息，因此导入文件并继续编辑的行为，应当与在其他人公开 Project 基础上继续编辑的行为相似。

### 保存 Asset

用户在编辑项目时，可以选择将当前项目中的某个 Asset 保存到素材库。在保存时，用户有机会指定 DisplayName、Category、IsPublic 等信息。
...
* 用户无法查看自己或公开的 Asset 列表，因此也无法从中选择并导入项目
* 关于用户身份
