...
* **27 门课**声明了 `apis`（第 1 课不涉及代码，无面板可收窄）；
* **28 门课**都有 `opening` 序列：每课一句 `prelude` 引导语（由旧的 `<course-prelude>` 内联而来）；**12 门课**含知识点视频步骤（step、turn、stepTo、turnTo、if、if/else、var、repeat、for in、waitUntil、distanceTo、Water）；**4 门课**含开场高亮——第 2 课「第一次运行」高亮运行按钮、第 3 课「量一量」与第 8 课「其实是数字」高亮尺子、第 4 课「自己写第一行」高亮 API 参考面板；
* **第 1 课**带 `<course-story-video>`（系列片头 `opening.webm`），进课前播放；
* 原提示词中的「新知识点（开场演示视频）」「本课涉及的 API」两节已删除（进入配置），`<course-prelude>` 内联进 `opening`，「完成判定」节按判定方式重写；其余教案内容（目标、当前代码、引导要点、彩蛋）原样保留。
