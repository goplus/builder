...
Stage Viewer 用于以视口的角度展示舞台内容，允许用户通过拖拽（空白区域或 Sprite）来调整视口位置。并提供打开舞台编辑器 Stage Editor 的入口。

### 舞台编辑器 Stage Editor

Stage Editor 提供更全面的舞台编辑功能，包括对整个地图的预览（支持缩放）、对地图尺寸的调整，以及选中精灵并对其基本属性（位置、转向等）进行就地编辑的能力。

Stage Editor 不提供对 Widget 的预览与编辑。
