...
//
// 为什么需要一个"大"队列：课程程序在调用 capability 期间是**阻塞**的——
// xgoexec.CallCapability 会一直等到前端把结果回传，而 Copilot 生成一次文本
// 可能要好几秒。这段时间里事件循环停摆，但学习者的游戏还在跑、还在 println，
// 这些日志会源源不断地投递进来。队列太小会立刻打满，而打满意味着丢失的可能
// 正是判定信号本身。
//
// 64 是个经验值：足够吸收一次 LLM 往返期间的日志突发，又不至于让"课程程序卡死"
// 这种真问题被无限缓冲掩盖过去。
