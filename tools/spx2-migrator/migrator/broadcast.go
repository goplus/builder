	...
	if ident, ok := call.Fun.(*xgoast.Ident); ok && matchFunctionName(ident.Name, "broadcast") {
		...
		if len(call.Args) == 2 {
			if isBooleanWaitParam(call.Args[1]) {
				return transformBroadcastWithWait(call, ident, call.Args[:1])
			}
		}
		// Handle `broadcast "msg", data, wait` - 3 arguments.
		if len(call.Args) == 3 {
			if isBooleanWaitParam(call.Args[2]) {
				return transformBroadcastWithWait(call, ident, call.Args[:2])
			}
		}
	}
...
func transformBroadcastWithWait(call *xgoast.CallExpr, ident *xgoast.Ident, newArgs []xgoast.Expr) bool {
