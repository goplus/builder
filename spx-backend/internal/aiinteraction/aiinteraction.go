...
func buildMessages(request *Request) ([]openai.ChatCompletionMessageParamUnion, error) {
	...
	for _, turn := range request.History {
		...
		// Add AI response message with command execution result if present.
		if turn.ResponseText != "" || turn.ResponseCommandName != "" {
			assistantMsg := turn.ResponseText

			// Add command execution information if present.
			if turn.ResponseCommandName != "" && turn.ExecutedCommandResult != nil {
				if assistantMsg != "" {
					assistantMsg += "\n\n"
				}

				// Build base function call information.
				var baseCallInfo string
				if len(turn.ResponseCommandArgs) > 0 {
					...
					baseCallInfo = fmt.Sprintf("Function call %s with arguments %s", turn.ResponseCommandName, string(argsJSON))
				} else {
					baseCallInfo = fmt.Sprintf("Function call %s", turn.ResponseCommandName)
				}

				// Build final message based on execution result.
				if turn.ExecutedCommandResult.Success {
					assistantMsg += fmt.Sprintf("[SUCCESS]: %s", baseCallInfo)
				} else {
					assistantMsg += fmt.Sprintf("[FAILED]: %s (%s)", baseCallInfo, turn.ExecutedCommandResult.ErrorMessage)
				}

				// Add interaction termination marker if applicable.
				if turn.ExecutedCommandResult.IsBreak {
					assistantMsg += " [Interaction terminated]"
				}
			}
