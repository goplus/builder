...
func TestBuildMessages(t *testing.T) {
	...
	t.Run("HistoryWithCommandExecution", func(t *testing.T) {
		...
		assert.Contains(t, content, "I'll attack now")
		assert.Contains(t, content, "[SUCCESS]: Function call Attack with arguments")
		assert.Contains(t, content, "target")
		assert.Contains(t, content, "goblin")
	})
	...
	t.Run("HistoryWithFailedCommands", func(t *testing.T) {
		...
		require.NotNil(t, assistantMsg)
		content := assistantMsg.Content.OfString.Value
		assert.Contains(t, content, "Attempting to open")
		assert.Contains(t, content, "[FAILED]: Function call OpenDoor")
		assert.Contains(t, content, "Door is locked")
	})
	...
	t.Run("HistoryWithBreakCommands", func(t *testing.T) {
		...
		require.NotNil(t, assistantMsg)
		content := assistantMsg.Content.OfString.Value
		assert.Contains(t, content, "Goodbye!")
		assert.Contains(t, content, "[SUCCESS]: Function call ExitGame")
		assert.Contains(t, content, "[Interaction terminated]")
	})
	...
	t.Run("CompleteScenario", func(t *testing.T) {
		...
		assert.Contains(t, historyAssistantMsg.Content.OfString.Value, "[SUCCESS]")

		turnMsg := messages[7].OfSystem
		require.NotNil(t, turnMsg)
		assert.Equal(t, "This is the initial response.", turnMsg.Content.OfString.Value)

		finalUserMsg := messages[8].OfUser
