...
function spawnSpxProcess() {
  ...
  if (process.platform !== 'win32') {
    ...
    //   // 尝试添加执行权限
    //   try {
    //     fs.chmodSync(spxExecutablePath, stat.mode | parseInt('111', 8))
    //     console.debug(`Added execute permission to ${spxExecutablePath}`)
    //   } catch (error) {
    //     console.debug(`Failed to add execute permission: ${error}`)
    //   }
    // }
  }
