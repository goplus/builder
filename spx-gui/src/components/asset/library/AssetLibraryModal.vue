...
watch(keyword, (v) => assetGenProps.value?.setInput(v))
watch(
  () => props.type,
  (type, _, onCleanup) => {
    assetGenProps.value = {
    ...
    onCleanup(() => assetGenProps.value?.dispose())
  },
  { immediate: true }
)
...
function handleGenerate() {
  ...
  if (gen == null) return
  if (gen.enrichState.state !== 'finished') {
    gen.enrich()
    return
  }
      ...
      <header class="header">
        ...
        <div class="recommended-buttons">
          <UIButton
            ...
            color="white"
            size="small"
            @click="searchInput += `${searchInput.length > 0 ? ' ' : ''}${$t(r.message)}`"
        ...
        >
          ...
          <ListResultWrapper :query-ret="queryRet" :height="436">
            <template v-if="SettingsInput != null && assetGenProps != null" #empty>
              <div class="empty" style="height: 436px">
                {{
                  $t({
                ...
                <SettingsInput :gen="assetGenProps">
                  <template #submit>
                    <EnrichableWrapper :enriched="assetGenProps.enrichState.state === 'finished'">
                      <UIButton :loading="assetGenProps.enrichState.state === 'running'" @click="handleGenerate">{{
                        $t({ en: 'Generate', zh: '生成' })
                      }}</UIButton>
                    </EnrichableWrapper>
                  </template>
            ...
            <!-- fixed ass et-list height to keep the layout stable -->
