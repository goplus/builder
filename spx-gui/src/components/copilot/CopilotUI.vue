...
import { assertNever, localStorageRef, until } from '@/utils/utils'
...
watch(
  ...
  async () => {
    ...
    panelPosition.value = copilot.active ? clampOpenPanelPosition() : clampClosePanelPosition()
  }
)
watch(
...
function startAnimation({ className, el }: { className: string; el?: HTMLElement }) {
  if (!el) {
    return () => {}
  }
...
function clampClosePanelPosition() {
  const { panelW, panelH, windowW, windowH } = fixResizeNullable()
  const { state, bottom } = getDirection(panelPosition.value, panelW)
...
function clampOpenPanelPosition() {
...
function openPanel() {
  const newPosition = clampOpenPanelPosition()
  if (!samePosition(panelPosition.value, newPosition)) {
    startAnimation({ className: 'animated', el: panelRef.value })
  }
...
const onDragMove = (offset: Offset) => {
  ...
  const swapThreshold = copilot.active ? panelW / 3 : panelW - panelW / 6
  const isOutOfBounds = position.right < -swapThreshold || position.right > windowW - panelW + swapThreshold
  triggerVisible.value = isOutOfBounds ? 'visible' : ''
  triggerState.value = newPosition.state
}
const onDragEnd = () => {
