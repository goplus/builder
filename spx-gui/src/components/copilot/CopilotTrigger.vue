...
import { ref } from 'vue'
import { cn, type ClassValue } from '@/components/ui'

const props = withDefaults(
  defineProps<{
    active?: boolean
    attachedTo?: 'left' | 'right' | null
    class?: ClassValue
  }>(),
  {
    active: false,
...
<template>
  <button
    ...
    :class="
      cn(
        'copilot-trigger group h-[50px] w-fit cursor-pointer border-0 rounded-[16px] bg-linear-to-r from-[#c390ff] to-[#72bbff] p-px shadow-lg',
        {
          active: props.active,
          'attached-to-left right-px rounded-l-none pl-0': props.attachedTo === 'left',
          'attached-to-right left-px rounded-r-none pr-0': props.attachedTo === 'right'
        },
        props.class
      )
    "
  ...
  >
    <span
      :class="
        cn('copilot-trigger-content flex h-full items-center rounded-[16px] bg-grey-100 px-1', {
          'rounded-l-none py-0 pr-1.25 pl-2.5': props.attachedTo === 'left',
          'rounded-r-none py-0 pr-2.5 pl-1.25': props.attachedTo === 'right'
        })
      "
    >
      <svg width="40" height="40" viewBox="0 0 40 40" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
        ...
        <defs>
          <linearGradient id="paint0_linear" x1="20" y1="0" x2="20" y2="40" gradientUnits="userSpaceOnUse">
            <stop
              :class="[
                '[stop-color:#9a77ff] group-hover:[stop-color:#ae92ff]',
                { '[stop-color:#ae92ff]': props.active }
              ]"
            />
            <stop
              offset="1"
              :class="[
                '[stop-color:#735ffa] group-hover:[stop-color:#9181fb]',
                { '[stop-color:#9181fb]': props.active }
              ]"
            />
          </linearGradient>
        </defs>
