...
-->
<template>
  <input type="file" accept=".zip" @change="add" />
    ...
    <div style="display: flex; flex-direction: column">
      <div>
        ...
        <template v-for="sprite in sprites" :key="sprite.name">
          <button
            :style="currentSpriteNames.includes(sprite.name) ? { color: 'blue' } : {}"
...
const toggleShowInStage = (name: string) => {
  currentSpriteNames.value = currentSpriteNames.value.includes(name)
...
const onDragEnd = (e: SpriteDragEndEvent) => {
  currentSprite.value = project.value.sprite.list.find(
    (sprite) => sprite.name === e.targets[0].sprite.name
  ) as Sprite
