...
function _godot_js_spx_on_action_just_pressed(action_name) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(147, 1, action_name);
...
function _godot_js_spx_on_axis_changed(action_name, value) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(150, 1, action_name, value);
...
function _godot_js_spx_on_collision_enter(self_id, other_id) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(151, 1, self_id, other_id);
...
function _godot_js_spx_on_engine_destroy() {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(154, 1);
 window.gdspx_on_engine_destroy();
}

function _godot_js_spx_on_engine_fixed_update(delta) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(155, 1, delta);
...
function _godot_js_spx_on_key_pressed(keyid) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(158, 1, keyid);
...
function _godot_js_spx_on_scene_sprite_instantiated(obj, type_name) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(162, 1, obj, type_name);
...
function _godot_js_spx_on_sprite_animation_changed(obj) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(163, 1, obj);
...
function _godot_js_spx_on_sprite_fixed_updated(delta) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(167, 1, delta);
...
function _godot_js_spx_on_sprite_updated(delta) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(173, 1, delta);
...
function _godot_js_spx_on_trigger_enter(self_id, other_id) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(175, 1, self_id, other_id);
...
function _godot_js_spx_on_ui_clicked(obj) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(178, 1, obj);
...
function _godot_js_spx_on_ui_text_changed(obj, text) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(184, 1, obj, text);
...
function _godot_js_spx_on_ui_toggle(obj, is_on) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(185, 1, obj, is_on);
 window.gdspx_on_ui_toggle(GodotRuntime.ToJsObj(obj), is_on);
}

function _godot_js_spx_on_ui_updated(obj) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(186, 1, obj);
 window.gdspx_on_ui_updated(GodotRuntime.ToJsObj(obj));
}

function _godot_js_tts_get_voices(p_callback) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(187, 1, p_callback);
 const func = GodotRuntime.get_func(p_callback);
 try {
  const arr = [];
...
function _godot_js_tts_is_paused() {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(188, 1);
...
function _godot_js_tts_speak(p_text, p_voice, p_volume, p_pitch, p_rate, p_utterance_id, p_callback) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(192, 1, p_text, p_voice, p_volume, p_pitch, p_rate, p_utterance_id, p_callback);
 const func = GodotRuntime.get_func(p_callback);
...
function _godot_js_update_files_cb(callback) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(194, 1, callback);
 const func = GodotRuntime.get_func(callback);
...
function _godot_js_websocket_buffered_amount(p_id) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(195, 1, p_id);
 return GodotWebSocket.bufferedAmount(p_id);
}

function _godot_js_websocket_close(p_id, p_code, p_reason) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(196, 1, p_id, p_code, p_reason);
 const code = p_code;
...
function _godot_js_websocket_create(p_ref, p_url, p_proto, p_on_open, p_on_message, p_on_error, p_on_close) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(197, 1, p_ref, p_url, p_proto, p_on_open, p_on_message, p_on_error, p_on_close);
...
function _godot_js_websocket_destroy(p_id) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(198, 1, p_id);
 GodotWebSocket.destroy(p_id);
}

function _godot_js_websocket_send(p_id, p_buf, p_buf_len, p_raw) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(199, 1, p_id, p_buf, p_buf_len, p_raw);
...
function _godot_js_wrapper_create_cb(p_ref, p_func) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(200, 1, p_ref, p_func);
...
function _godot_js_wrapper_create_object(p_object, p_args, p_argc, p_convert_callback, p_exchange, p_lock, p_free_lock_callback) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(201, 1, p_object, p_args, p_argc, p_convert_callback, p_exchange, p_lock, p_free_lock_callback);
...
function _godot_js_wrapper_interface_get(p_name) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(202, 1, p_name);
 const name = GodotRuntime.parseString(p_name);
 if (typeof window[name] !== "undefined") {
...
function _godot_js_wrapper_object_call(p_id, p_method, p_args, p_argc, p_convert_callback, p_exchange, p_lock, p_free_lock_callback) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(203, 1, p_id, p_method, p_args, p_argc, p_convert_callback, p_exchange, p_lock, p_free_lock_callback);
...
function _godot_js_wrapper_object_get(p_id, p_exchange, p_prop) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(204, 1, p_id, p_exchange, p_prop);
 const obj = GodotJSWrapper.get_proxied_value(p_id);
 if (obj === undefined) {
  return 0;
 }
...
function _godot_js_wrapper_object_getvar(p_id, p_type, p_exchange) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(205, 1, p_id, p_type, p_exchange);
 const obj = GodotJSWrapper.get_proxied_value(p_id);
 if (obj === undefined) {
  return -1;
 }
...
function _godot_js_wrapper_object_set(p_id, p_name, p_type, p_exchange) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(206, 1, p_id, p_name, p_type, p_exchange);
 const obj = GodotJSWrapper.get_proxied_value(p_id);
 if (obj === undefined) {
  return;
 }
...
function _godot_js_wrapper_object_set_cb_ret(p_val_type, p_val_ex) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(207, 1, p_val_type, p_val_ex);
...
function _godot_js_wrapper_object_setvar(p_id, p_key_type, p_key_ex, p_val_type, p_val_ex) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(208, 1, p_id, p_key_type, p_key_ex, p_val_type, p_val_ex);
 const obj = GodotJSWrapper.get_proxied_value(p_id);
 if (obj === undefined) {
  return -1;
 }
...
function _godot_js_wrapper_object_unref(p_id) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(209, 1, p_id);
...
function _godot_webgl2_glFramebufferTextureMultiviewOVR(target, attachment, texture, level, base_view_index, num_views) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(210, 1, target, attachment, texture, level, base_view_index, num_views);
...
function _godot_webgl2_glGetBufferSubData(target, offset, size, data) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(211, 1, target, offset, size, data);
 const gl_context_handle = _emscripten_webgl_get_current_context();
 const gl = GL.getContext(gl_context_handle);
 if (gl) {
...
function _godot_webxr_get_bounds_geometry(r_points) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(212, 1, r_points);
...
function _godot_webxr_get_color_texture() {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(213, 1);
...
function _godot_webxr_get_projection_for_view(p_view, r_transform) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(216, 1, p_view, r_transform);
...
function _godot_webxr_get_render_target_size(r_size) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(217, 1, r_size);
 const subimage = GodotWebXR.getSubImage();
 if (subimage === null) {
  return false;
 }
...
function _godot_webxr_get_supported_frame_rates(r_frame_rates) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(218, 1, r_frame_rates);
...
function _godot_webxr_get_transform_for_view(p_view, r_transform) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(219, 1, p_view, r_transform);
 if (!GodotWebXR.session || !GodotWebXR.pose) {
  return false;
 }
...
function _godot_webxr_get_velocity_texture() {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(220, 1);
 const subimage = GodotWebXR.getSubImage();
 if (subimage === null) {
  return 0;
 }
...
function _godot_webxr_initialize(p_session_mode, p_required_features, p_optional_features, p_requested_reference_spaces, p_on_session_started, p_on_session_ended, p_on_session_failed, p_on_input_event, p_on_simple_event) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(223, 1, p_session_mode, p_required_features, p_optional_features, p_requested_reference_spaces, p_on_session_started, p_on_session_ended, p_on_session_failed, p_on_input_event, p_on_simple_event);
...
function _godot_webxr_is_session_supported(p_session_mode, p_callback) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(224, 1, p_session_mode, p_callback);
 const session_mode = GodotRuntime.parseString(p_session_mode);
...
function _godot_webxr_is_supported() {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(225, 1);
 return !!navigator.xr;
}
...
function _godot_webxr_update_input_source(p_input_source_id, r_target_pose, r_target_ray_mode, r_touch_index, r_has_grip_pose, r_grip_pose, r_has_standard_mapping, r_button_count, r_buttons, r_axes_count, r_axes) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(227, 1, p_input_source_id, r_target_pose, r_target_ray_mode, r_touch_index, r_has_grip_pose, r_grip_pose, r_has_standard_mapping, r_button_count, r_buttons, r_axes_count, r_axes);
...
function _godot_webxr_update_target_frame_rate(p_frame_rate) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(228, 1, p_frame_rate);
