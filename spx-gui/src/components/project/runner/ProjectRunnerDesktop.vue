...
onMounted(() => {
  ...
  spxGuiDesktop.onGameExit((code: number) => {
    console.debug('ProjectRunner game exited with code:', code)
    exited.value = true
    emit('exit', code)
  })
})
...
async function runNative(ctrl: AbortController) {
  ...
  try {
    ...
    const [zipped, aiDescription] = await Promise.all([
      ...
      isProjectUsingAIInteraction(props.project) ? props.project.ensureAIDescription(false, ctrl.signal) : Promise.resolve(null)
    ])
