...
const handleInputUpdate = useMessageHandle(
  (newInput: Input) => {
    ...
    if (td == null) return
    let newCode: string
    if (newInput.kind === InputKind.Predefined) {
      newCode = newInput.name
    } else {
      const handler = props.controller.provider?.provideInputTypeHandler(newInput.type)
      if (handler == null) throw new Error('No handler for input type ' + newInput.type)
      const expr = handler.exprForInput(newInput)
      if (expr == null) throw new Error('Invalid input')
      newCode = expr
    }
    td.pushEdits([
      {
