...
async function handleOpenEyeDropper() {
  ...
  alpha.value = 100 // Reset alpha to fully opaque when picking a color
}
...
<template>
  <div class="spx-color-input">
    <section class="sliders">
      ...
      <div class="slider-wrapper">
        <h5 class="slider-title">
          {{ $t({ en: 'Alpha: ', zh: '不透明度：' }) }}<span class="slider-value">{{ alpha }}</span>
        </h5>
        <ColorSlider
          v-model:value="alpha"
          :get-color="(v: number) => builderHSBA2CSSColorString([hue, saturation, brightness, v])"
        />
      </div>
    </section>
    <UIDivider />
    <section class="inputs">
      ...
      <UINumberInput v-model:value="alpha" class="input" :min="0" :max="100" :step="1" @keyup.enter="handleSubmit">
