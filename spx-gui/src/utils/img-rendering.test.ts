...
describe('font-aware image rendering', () => {
  ...
  it('reuses a rendered image for the same source File', async () => {
    const file = fromText(
      'costume.svg',
      '<svg xmlns="http://www.w3.org/2000/svg"><text font-family="custom">hello</text></svg>',
      {
        type: 'image/svg+xml'
      }
    )
  ...
  it('uses the original URL when no font context is provided', async () => {
    const file = fromText('costume.svg', '<svg xmlns="http://www.w3.org/2000/svg"><text>hello</text></svg>', {
      type: 'image/svg+xml'
    })
  ...
  it('passes project font preferences to SVG rendering', async () => {
    const file = fromText('costume.svg', '<svg xmlns="http://www.w3.org/2000/svg"><text>你好</text></svg>', {
      type: 'image/svg+xml'
    })
    ...
    expect(render).toHaveBeenCalledWith(expect.stringContaining('svg { font-family: "basic-chinese", "default"; }'), {
      maxSize: 1024
    })
  })
