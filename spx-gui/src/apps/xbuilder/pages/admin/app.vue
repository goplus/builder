        ...
        <div class="flex min-w-0 items-center gap-4">
          ...
          <div class="min-w-0">
            ...
            <div class="mt-0.5 flex min-w-0 items-center gap-1 font-mono text-sm text-grey-800">
              ...
              <UICopyButton :value="app.name" :label="{ en: 'Copy app name', zh: '复制应用名称' }" />
            </div>
            <div class="mt-1 text-xs text-grey-700">
              <span class="flex items-center gap-1">
                <span>{{ $t({ en: 'Client ID', zh: '客户端 ID' }) }}: {{ app.id }}</span>
                <UICopyButton :value="app.id" :label="{ en: 'Copy client ID', zh: '复制客户端 ID' }" />
              </span>
            </div>
          </div>
        </div>
            ...
            <form class="flex flex-col gap-5" @submit.prevent="handleUpdateEndpoints.fn">
              <label class="flex flex-col gap-1 text-sm text-grey-900">
                ...
                <span class="text-xs text-grey-700">
                  {{
                    $t({
                      en: 'One URI per line. Values are matched exactly; production web callbacks must use HTTPS.',
                      zh: '每行一个 URI。服务端进行精确匹配；生产环境 Web 回调必须使用 HTTPS。'
                    })
                  }}
                </span>
              </label>
              <label class="flex flex-col gap-1 text-sm text-grey-900">
                ...
                <span class="text-xs text-grey-700">
                  {{
                    $t({
                      en: 'One origin per line, such as https://example.com. Do not include a path, query, or fragment.',
                      zh: '每行一个 Origin，例如 https://example.com；不要包含路径、查询参数或 fragment。'
                    })
                  }}
                </span>
              </label>
                ...
                >
                  <div class="min-w-0 text-sm">
                    ...
                    <div class="mt-1 flex items-center gap-1 font-mono text-xs text-grey-700">
                      ...
                      <UICopyButton :value="secret.id" :label="{ en: 'Copy secret ID', zh: '复制密钥 ID' }" />
                    </div>
                    <div class="mt-1 text-xs text-grey-700">
...
import { UIButton, UICopyButton, UIError, UILoading, UISwitch, UITextInput } from '@/components/ui'
