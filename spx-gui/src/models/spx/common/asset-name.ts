...
import { getApiStringLength, lowFirst, unicodeSafeSlice, upFirst } from '@/utils/utils'
...
function validateAssetName(name: string) {
  ...
  if (getApiStringLength(name) > assetNameMaxLength)
    return {
...
export function validateAssetDisplayName(name: string) {
  ...
  if (getApiStringLength(name) > assetDisplayNameMaxLength)
    return {
