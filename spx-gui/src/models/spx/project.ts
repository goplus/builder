...
import { getApiStringLength, unicodeSafeSlice } from '@/utils/utils'
...
const aiDescriptionTruncationNoticeLength = getApiStringLength(aiDescriptionTruncationNotice)
...
export class SpxProject extends Disposable implements IProject {
  ...
  private async serializeForAI() {
    ...
    const append = (value: string) => appendKnownLength(value, getApiStringLength(value))
    const appendLine = (line: string) => append(`${line}\n`)
    const appendBlankLine = () => append('\n')

    const appendBody = (body: string) => {
      const bodyLength = getApiStringLength(body)
    ...
    const appendSection = async (title: string, file: File, maxBodyLength?: number) => {
      ...
      const headerLength = getApiStringLength(headerLine)
    ...
    if (skippedSections.length > 0 && remaining > 0) {
      ...
      if (remaining > getApiStringLength(`${header}\n`)) {
        appendLine(header)
